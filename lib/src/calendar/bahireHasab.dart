...
class BahireHasab extends Equatable {
  ...
  /// provided.
  ///
  int get ameteAlem => _ameteFida + _year;
  ...
  Map<String, dynamic> get nenewe {
    var meskerem1 = getMeskeremOne(returnName: true);
    var month = yebealeMetkihWer();
    var date;
    var dayTewsak;
    ...
    var monthName = dayTewsak + metkih > 30 ? 'የካቲት' : 'ጥር';
    if (month == 2) {
      // ጥቅምት
      monthName = 'የካቲት';
      var tikimt1 = _weekdays[(_weekdays.indexOf(meskerem1) + 2) % 7];
      var metkihElet = _weekdays[(_weekdays.indexOf(tikimt1) + metkih - 1) % 7];
      _yeeletTewsak.forEach((el) => {
  ...
  Map<String, dynamic> getSingleBealOrTsom(String name) {
    try {
      var status = isMovableHoliday(name);
      if (status) {
        var mebajaHamer = nenewe;
        var target = _yebealTewsak[name];
        var a = {
          'month': _months[_months.indexOf(mebajaHamer['month']) +
